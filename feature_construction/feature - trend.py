...
row2=None
row3=None
app_train = pd.read_csv('./application_train.csv', nrows=row1).sort_values('SK_ID_CURR')
app_test = pd.read_csv('./application_test.csv', nrows=row1).sort_values('SK_ID_CURR')
bureau = pd.read_csv('./bureau.csv', nrows=row2).sort_values(['SK_ID_CURR', 'SK_ID_BUREAU'])
bureau_balance = pd.read_csv('./bureau_balance.csv', nrows=row3).sort_values(['SK_ID_BUREAU', 'MONTHS_BALANCE'])
previous = pd.read_csv('./previous_application.csv', nrows=row3).sort_values(['SK_ID_CURR', 'SK_ID_PREV'])
cash = pd.read_csv('./POS_CASH_balance.csv', nrows=row3).sort_values(['SK_ID_CURR', 'SK_ID_PREV', 'MONTHS_BALANCE'])
credit = pd.read_csv('./credit_card_balance.csv', nrows=row3).sort_values(['SK_ID_CURR', 'SK_ID_PREV', 'MONTHS_BALANCE'])
installments = pd.read_csv('./installments_payments.csv', nrows=row3).sort_values(['SK_ID_CURR', 'SK_ID_PREV'])
...
bureau_balance.drop(['MONTHS_BALANCE'],axis=1)
# Make a date column
...
cash.drop(['MONTHS_BALANCE'],axis=1)
# # credit
...
credit.drop(['MONTHS_BALANCE'],axis=1)
# installment
...
es = es.add_relationships([r_app_bureau, r_test_app_bureau,
						   r_app_previous, r_test_app_previous,
						   r_previous_installments])
						   

del app_train, app_test, bureau, previous, installments
gc.collect()
...
time_features.reset_index().to_csv('Output/trend3_train.csv', index=False)
# test features
